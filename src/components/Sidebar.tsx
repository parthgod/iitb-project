...
const Sidebar = () => {
  ...
  return (
    <div
      className={`h-screen bg-[#1E40AF] text-white transition-all duration-200 ease-in-out pl-3 ${isOpen ? "w-1/6 overflow-auto" : "w-14 overflow-hidden"} border-r-0 border-gray-200 scrollbar-hide`}
    >
      <input
      ...
      >
        {sideMenu?.map((item: any, ind: any) => (
          ...
          >
            <div
              className={`flex group justify-start items-center gap-2 hover:cursor-pointer hover:text-black transition-color w-full duration-300 p-2 pl-2 ease-in-out active:text-lg ${pathname.toString() === item.route.toString() ? "tab bg-gray-200" : "text-white"}`}
            >
