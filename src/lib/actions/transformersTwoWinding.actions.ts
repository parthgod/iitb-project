  ...
  try {
    ...
    if (response) {
      ...
      modificationHistory = {
        ...
        operationType: "Create",
        date: new Date(),
        message: `<span style="font-weight: 610">${data.length}</span> records were added to <span style="font-weight: 610">Transformers Two Winding</span> from an excel file.`,
        document: {
...
export const toggleTransformersTwoWindingSwitchValue = async (
  id: string,
  column: IColumn,
  userId: string,
  value: "ON" | "OFF"
  ...
  try {
    ...
    let response;
    if (column.isDefault) {
      response = await TransformersTwoWinding.findByIdAndUpdate(id, { [column.field]: value });
    } else {
      const originalTransformersTwoWinding = await TransformersTwoWinding.findById(id);
      const additionalFields = { ...originalTransformersTwoWinding.additionalFields, [column.field]: value };
      response = await TransformersTwoWinding.findByIdAndUpdate(id, additionalFields);
    }
    let modificationHistory = {
      ...
      operationType: "Update",
      date: new Date(),
      message: `<span style="font-weight: 610">${column.title}'s</span> switch status was set to <span style="font-weight: 610">${value}</span> for record <span style="font-weight: 610">${id}</span> in <span style="font-weight: 610">Transformers Two Winding</span> table`,
      document: {
