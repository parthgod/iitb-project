  ...
  try {
    ...
    if (response) {
      ...
      modificationHistory = {
        ...
        operationType: "Create",
        date: new Date(),
        message: `<span style="font-weight: 610">${data.length}</span> records were added to <span style="font-weight: 610">LCC-HVDC Link</span> from an excel file.`,
        document: {
...
export const toggleLCCHVDCLinkSwitchValue = async (
  id: string,
  column: IColumn,
  userId: string,
  value: "ON" | "OFF"
  ...
  try {
    ...
    let response;
    if (column.isDefault) {
      response = await LCCHVDCLink.findByIdAndUpdate(id, { [column.field]: value });
    } else {
      const originalLCCHVDCLink = await LCCHVDCLink.findById(id);
      const additionalFields = { ...originalLCCHVDCLink.additionalFields, [column.field]: value };
      response = await LCCHVDCLink.findByIdAndUpdate(id, additionalFields);
    }
    let modificationHistory = {
      ...
      operationType: "Update",
      date: new Date(),
      message: `<span style="font-weight: 610">${column.title}'s</span> switch status was set to <span style="font-weight: 610">${value}</span> for record <span style="font-weight: 610">${id}</span> in <span style="font-weight: 610">LCC-HVDC Link</span> table`,
      document: {
...
export const deleteManyLCCHVDCLink = async (recordsToDelete: string[], userId: string, path: string) => {
  try {
    await connectToDatabase();

    const response = await LCCHVDCLink.deleteMany({ _id: { $in: recordsToDelete } });
    if (response) {
      let modificationHistory: any;
      modificationHistory = {
        userId: new ObjectId(userId),
        ...
        operationType: "Delete",
        date: new Date(),
        message: `<span style="font-weight: 610">${recordsToDelete.length}</span> records were deleted from <span style="font-weight: 610">LCC-HVDC Link</span>.`,
        document: {
      ...
      await ModificationHistory.create(modificationHistory);
      revalidatePath(path);
      return { data: `${recordsToDelete.length} records were deleted successfully from LCC-HVDC Link.`, status: 200 };
    }
  } catch (error) {
    throw new Error(typeof error === "string" ? error : JSON.stringify(error));
  }
