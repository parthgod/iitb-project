...
export const getAllModificationsHistory = async ({
  ...
  databaseName: string;
  query: string;
}): Promise<{ data: IModificationHistory[]; status: number; totalDocuments: number; conditions: any }> => {
  try {
    ...
    if (query) {
      ...
      else return { data: [], status: 200, totalDocuments: 0, conditions };
    }
    ...
    return { data: JSON.parse(JSON.stringify(response)), status: 200, totalDocuments, conditions };
  } catch (error) {
